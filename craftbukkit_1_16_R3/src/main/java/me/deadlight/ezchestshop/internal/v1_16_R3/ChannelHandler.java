            ...
            if (Utils.activeOutlines.containsKey(entityID)) {
                ...
                EzChestShop.getPlugin().getServer().getScheduler().runTaskLater(
                        EzChestShop.getPlugin(), () -> {
                            Bukkit.getPluginManager().callEvent(new PlayerInteractEvent(player, Action.RIGHT_CLICK_BLOCK, player.getItemInHand(), outline.block, outline.block.getFace(outline.block), null));
                        }, 1L);
            }
        }
